...
for (let i = 0; i < select.length; i++) {
  select[i].addEventListener('click', function () {
    ...
    showHide()
    window.location.replace('./?negara=' + this.getAttribute('country-id'))
  })
}
...
const formatNum = (() => {
  const num = document.querySelectorAll('.format')
  num.forEach(function (number) {
    const newFormat = number.innerText
      .toString()
      .replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1.')
    this.innerText = newFormat
  })
})()
